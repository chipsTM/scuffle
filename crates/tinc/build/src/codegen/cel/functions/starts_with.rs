...
// this.startsWith(arg) -> arg in this
impl Function for StartsWith {
    const NAME: &'static str = "startsWith";
    ...
    fn compile(ctx: CompilerCtx) -> Result<CompiledExpr, CompileError> {
        let Some(this) = &ctx.this else {
            return Err(CompileError::MissingTarget {
                func: Self::NAME,
                message: "this is required when calling the startsWith function".to_string(),
            });
        };
        ...
        if !arg.ty.can_be_cel() {
            return Err(CompileError::TypeConversion {
                ...
                message: "the startsWith function can only be called with CEL value argument types".to_string(),
            });
        }
    ...
    fn interpret(fctx: &FunctionContext) -> Result<cel_interpreter::Value, ExecutionError> {
        ...
        match (this, arg) {
            (cel_interpreter::Value::String(s), cel_interpreter::Value::String(t)) => {
                Ok(cel_interpreter::Value::Bool(s.starts_with(t.as_str())))
            }
            (cel_interpreter::Value::Bytes(s), cel_interpreter::Value::Bytes(t)) => {
                Ok(cel_interpreter::Value::Bool(s.starts_with(t.as_slice())))
            }
